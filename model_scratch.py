...
class CNN(nn.Module):
	def __init__(self, batch_size, output_size, in_channels, out_channels, kernel_heights, stride, padding, keep_probab, 
		...
		self.word_embeddings.weight = nn.Parameter(torch.FloatTensor(weights), requires_grad=fine_tune)
...
class CNN_Multichannel(nn.Module):
	def __init__(self, batch_size, output_size, in_channels, out_channels, kernel_heights, stride, padding, keep_probab, 
		...
		self.fixed_embeddings.weight = nn.Parameter(torch.FloatTensor(weights), requires_grad=False)
		self.tuned_embeddings = nn.Embedding(vocab_size, embedding_length)
		self.tuned_embeddings.weight = nn.Parameter(torch.FloatTensor(weights), requires_grad=True)
